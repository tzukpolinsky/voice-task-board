...
def is_daily(rrule_str: str) -> bool:
    """
    Check if an RRULE is daily with no large INTERVAL.
    
    Treat INTERVAL >= 2 as non-daily for catch-up purposes.
    
    Args:
        rrule_str: RRULE string
    
    Returns:
        True if FREQ=DAILY and INTERVAL not specified or == 1, else False.
    """
    if not rrule_str:
        return False
    
    try:
        # Parse the RRULE to extract its parameters
        ...
        for part in parts:
            if part.startswith("FREQ="):
                ...
                interval_part = int(part.split("=")[1])
        
        if freq_part != "DAILY":
            return False
        
        # If INTERVAL is specified and >= 2, treat as non-daily
        if interval_part is not None and interval_part >= 2:
            return False
